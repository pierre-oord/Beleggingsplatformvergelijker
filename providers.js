/* global window */
(() => {
  /**
   * Provider config format
   * - fees.components: array van fee-componenten (fixed, percentOfBase, etc.)
   * - percentOfBase.ratePct: altijd op jaarbasis (%)
   * - percentOfBase.frequency: wanneer in rekening gebracht. "monthly" = 1/12 per maand, "quarterly" = 1/4 in maanden 3,6,9,12
   * - percentOfBase.is_compounded: true = samengestelde rente, false/niet gezet = lineaire benadering (jaarbedrag/12 of /4)
   * - percentOfBase.minimumAnnual: optioneel minimum jaarbedrag voor dit component
   * - percentOfBase.minimumMonthly: optioneel minimum per maand voor dit component
   * - dividend: 'accumulating' | 'distributing' | null
   * - strategy: 'active' | 'passive' | null
   * - underlyingDepositPercentage: percentage inlegkosten op storting (en initInleg in maand 1), standaard 0
   * - underlyingWithdrawalPercentage: percentage opnamekosten op opname, standaard 0
   * - tax_BE_roerende_voorheffing_percentage: optioneel percentage (number) of null
   * - transactions.deposit.bill_begin_month: true = tx kosten storting vóór TOB in rekening. false/niet gezet = tx later (na aanbiederkosten).
   * - Keep it declarative so providers are easy to add.
   */
  const PROVIDERS = [
    {
      id: "uptomore",
      name: "UpToMore",
      website: "https://www.uptomore.com",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: "16-02-2026",
      // Extra eigenschappen (informational only; no calculations depend on these)
      minimumInleg: null, // EUR
      duurzaamheid: "Lichtgroen", // 'Grijs' | 'Lichtgroen' | 'Donkergroen'
      aandelenpercentage: 100, // 0..100 (%)
      fbi: false,
      risk_indicator: null,
      dividend: "accumulating",
      strategy: "passive",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Nee", // Je koopt bij UpToMore niet direct ETF's op je eigen naam.
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: "Nee", // UpToMore keert geen dividenden uit.
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Nee", // Aandelen staan direct op naam.
      tax_BE_reynderstaks: "Nee",
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.2064,
      },
      underlyingCostBasis: "avg", // begin | avg | end
      fixedMonthlySkipMonths: 3, // Eerste 3 maanden geen fixed kosten
      fees: {
        components: [
          { kind: "fixed", amount: 0.99 },
          { kind: "percentOfBase", ratePct: 0.1, frequency: "monthly", basis: "avg" },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "easyvest-algemeen",
      name: "Easyvest (Algemeen beleggen Portefeuille 10)",
      website: "https://www.easyvest.be",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["BE"],
      lastUpdated: "17-02-2026",
      minimumInleg: 5000,
      duurzaamheid: "Grijs",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: "accumulating",
      strategy: "passive",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "Nee",
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      service_recurring_investing: true,
      service_fractional_investing: false,
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: { underlyingAnnualPct: 0.17 },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "end",
            frequency: "monthly",
            tiers: {
              inclusiveUpper: false,
              apply: "highest",
              tiers: [
                { upTo: 25000, ratePct: 1.0 },
                { upTo: 50000, ratePct: 0.9 },
                { upTo: 100000, ratePct: 0.75 },
                { upTo: 250000, ratePct: 0.6 },
                { upTo: 1000000, ratePct: 0.5 },
                { upTo: 2500000, ratePct: 0.45 },
                { upTo: null, ratePct: 0.4 },
              ],
            },
          },
        ],
      },
      transactions: { deposit: null, withdraw: null },
    },
    {
      id: "easyvest-klimaatportefeuille",
      name: "Easyvest (Klimaatportefeuille Portefeuille 10)",
      website: "https://www.easyvest.be",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["BE"],
      lastUpdated: "17-02-2026",
      minimumInleg: 5000,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: "accumulating",
      strategy: "passive",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "Nee",
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      service_recurring_investing: true,
      service_fractional_investing: false,
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: { underlyingAnnualPct: 0.20 },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "end",
            frequency: "monthly",
            tiers: {
              inclusiveUpper: false,
              apply: "highest",
              tiers: [
                { upTo: 25000, ratePct: 1.0 },
                { upTo: 50000, ratePct: 0.9 },
                { upTo: 100000, ratePct: 0.75 },
                { upTo: 250000, ratePct: 0.6 },
                { upTo: 1000000, ratePct: 0.5 },
                { upTo: 2500000, ratePct: 0.45 },
                { upTo: null, ratePct: 0.4 },
              ],
            },
          },
        ],
      },
      transactions: { deposit: null, withdraw: null },
    },
    {
      id: "meesman-aandelen-wereldwijd-zeer-offensief",
      name: "Meesman (Aandelen wereldwijd, zeer offensief)",
      website: "https://www.meesman.nl",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: "17-02-2026",
      // Extra eigenschappen (informational only; no calculations depend on deze velden)
      minimumInleg: null, // Die is 10.000 maar je mag er naartoe sparen...
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: true,
      risk_indicator: null,
      dividend: "distributing",
      strategy: "passive",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        // 10-03-2026:
        // Aaandelen wereldwijd A hebben deze fondsen (ontvangen per email van Meesman)
        // IE00BF2PF682 0,07% lopende + 0,01% Transactiekosten (82,6% van fonds 2025 jaarrekening)
        // NL0013089147 0,18% lopende + 0,07% Transactiekosten (8,2% van fonds 2025 jaarrekening)
        // NL0013552094 0,23% Lopende + 0,05% Transactiekosten (9,2 van fonds 2025 jaarrekening)
        // Dit geeft een gemiddelde van 0,11% onderliggende kosten, terwijl Meesman 0,09 noemt. 
        // In het KID van 31-12-2026 (PDF datum 06-02-2026, 09:45:48) wordt 0,40% beheerkosten en 0,02% transactiekosten vermeld.
        //
        // Dividendlekkagevoordeel
        // Diverse aanbieders roepen grote getallen, maar onderbouwen die niet. Back-of-the-enveloppe berekningen 
        // laten zien dat die niet kunnen kloppen. Er zouden daarnaast grote verschillen moeten zitten tussen de diverse fondsen.
        // Bij Meesman heb ik als enige aanbieder iets in de jaarrekening kunnen vinden, maar niet bevestigd gekregen of dit klopt.
        // Als Meesman bijvoorbeeld van de afgelopen 5 jaar het bespaarde dividendlekkage percentage kan overleggen,
        // kunnen we dit opnemen in de tool. Hierna volgt een stuk uit de jaarrrekeningen:
        //
        // Aandelen Wereldwijd Totaal (A)	
        //                                              2024	                2023
        // Reservering te verrekenen dividendbelasting 	1.472.723	            865.365
        // Te verrekenen dividendbelasting	            470.255	              250.622
        // Beleggingsfondsen	                          918.950.834	          536.051.105
        //
        // 1.472.723 + 470.255 / ((918.950.834 +	536.051.105)/2) = 0,27%
        // Nu moet je nog rekening houden dat je pas na één jaar dit bedrag ontvangt van Meesman.
        // En dat je van de belastingdienst ook nog 6 maanden moet wachten totdat je de teruggave krijgt.
        // 
        // Een ander issue dat (denk ik) nog speelt: als je niet weet wat een fonds aan belasting betaalt,
        // kun je de teruggave ook niet goed plaatsen.
        // Stel dat je een fonds hebt met heel bedrijven die vooral dividend betalen. Logischerwijs is er dan ook meer
        // dividendlekkage voordeel. Hetzelfde geld voor fondsen die relatief veel belasting betalen door welke oorzaak dan ook.
        // Al met al: ik kom uit op een potentiële 0,13% voordeel, mits alle bovenstaande aannames juist zijn.

        underlyingAnnualPct: 0.11,
      },
      // Aanbiederkosten op basis van saldo (S4, annual percentage):
      // 0,31% tot saldo 1.000.000, vanaf 1.000.000: 0,16%.
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "avg",
            frequency: "monthly",
            is_compounded: false,
            tiers: {
              inclusiveUpper: false,
              apply: "highest",
              tiers: [
                { upTo: 1000000, ratePct: 0.31 },
                { upTo: null, ratePct: 0.16 },
              ],
            },
          },
        ],
      },
      // Transactiekosten: 0,25% per aankoop/verkoop.
      // Vanaf een "saldo" van 500.000 vervallen de transactiekosten.
      transactions: {
        deposit: {
          kind: "tieredTransaction",
          bill_begin_month: true,
          // 0,25% per transactie zolang saldo < 500.000; daarboven geen tx-kosten.
          balanceThresholdForFees: 500000,
          percentRatePct: 0.25,
        },
        withdraw: {
          kind: "tieredTransaction",
          balanceThresholdForFees: 500000,
          percentRatePct: 0.25,
        },
      },
    },
    {
      id: "curvo-growth",
      name: "Curvo (Growth)",
      website: "https://curvo.eu",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["BE"],
      lastUpdated: "17-02-2026",
      minimumInleg: 50,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: "accumulating",
      strategy: "passive",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Nee",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: "Nee", // Curvo heeft alleen accumulerende fondsen
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja", // Als je het geluk hebt zo'n bedrag te hebben, laat het ons weten en wij helpen je met de aangifte ervan.
      tax_BE_reynderstaks: "Nee", //  de belasting geldt alleen voor de winst op obligatiefondsen, niet op aandelenfondsen. Dat betekent dat als je in de Growth-portefeuille (100% aandelen) belegt, je geen Reynders-taks betaalt.
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: { underlyingAnnualPct: 0.21 },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "end",
            frequency: "quarterly",
            tiers: {
              inclusiveUpper: false,
              apply: "marginal",
              tiers: [
                { upTo: 50000, ratePct: 1 },
                { upTo: 100000, ratePct: 0.85 },
                { upTo: 250000, ratePct: 0.7 },
                { upTo: null, ratePct: 0.6 },
              ],
            },
          },
        ],
      },
      transactions: { deposit: null, withdraw: null },
    },
    {
      id: "medirect-memanaged-groei",
      name: "MeDirect (MeManaged Groei)",
      website: "https://www.medirect.be",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL", "BE"],
      lastUpdated: "19-02-2026",
      minimumInleg: 5000,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 90,
      fbi: false,
      risk_indicator: null,
      dividend: "distributing",
      strategy: "active", // MeDirect maakt gebruik van de volatiliteits beheerde portefeuilles van BlackRock. Je portefeuille wordt actief beheerd binnen het risicoprofiel dat je kiest. Dit gebeurt op basis van de bewegelijkheid in de markt. Uniek is dat de verhouding tussen aandelen en obligaties flexibel blijft. 
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Nee",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: "Ja",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: 30,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee", // alleen als er >10% in vast rentende obligaties wordt belegd in de ETFs
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.4,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "end",
            frequency: "quarterly",
            ratePct: 0.9,
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "medirect-mesolo",
      name: "MeDirect (MeSolo)",
      website: "https://www.medirect.be",
      type: "broker",
      is_bank: false,
      countries: ["NL", "BE"],
      lastUpdated: "19-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: false,
      service_fractional_investing: false,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "end",
            frequency: "quarterly",
            ratePct: 0.30,
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "degiro",
      name: "DEGIRO",
      website: "https://www.degiro.com",
      type: "broker",
      is_bank: false,
      countries: ["NL", "BE", "DE", "AT", "FR", "IT", "ES", "PL", "CZ", "HU", "IE", "UK"],
      lastUpdated: null,
      // Extra eigenschappen (informational only; no calculations depend on these)
      minimumInleg: null, // EUR
      duurzaamheid: null, // 'Grijs' | 'Lichtgroen' | 'Donkergroen'
      aandelenpercentage: null, // 0..100 (%)
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      service_recurring_investing: false,
      service_fractional_investing: false,
      // Belgische TOB op inleg (storting/initiële inleg)
      tax_BE_TOB_percentage: 0.12, // 0,12% per inleg
      tax_BE_TOB_max: 1300, // maximaal 1.300 EUR per maand (cap)
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: { kind: "fixedPerTransaction", amount: 1.0 },
        withdraw: { kind: "fixedPerTransaction", amount: 1.0 },
      },
    },
    {
      id: "lynx",
      name: "LYNX",
      website: "https://www.lynx.be",
      type: "broker",
      is_bank: false,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: false,
      service_fractional_investing: false,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: {
          kind: "tieredTransaction",
          percentRatePct: 0.09,
          minimumAmount: 6,
          maximumAmount: 99,
        },
        withdraw: {
          kind: "tieredTransaction",
          percentRatePct: 0.09,
          minimumAmount: 6,
          maximumAmount: 99,
        },
      },
    },
    {
      id: "fitvermogen",
      name: "Fitvermogen",
      website: "https://www.fitvermogen.nl",
      type: "broker",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: "25-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: "",
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: false,
      service_fractional_investing: false,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            ratePct: 0.2,
            frequency: "quarterly",
            basis: "avg",
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "easybroker",
      name: "Easybroker",
      website: "https://www.easybroker.nl",
      type: "broker",
      is_bank: false,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: {
          kind: "tieredTransaction",
          percentRatePct: 0.08,
          minimumAmount: 3,
        },
        withdraw: {
          kind: "tieredTransaction",
          percentRatePct: 0.08,
          minimumAmount: 3,
        },
      },
    },
    {
      id: "trade-republic",
      name: "Trade Republic",
      website: "https://www.traderepublic.com",
      type: "broker",
      is_bank: true,
      countries: ["BE", "NL", "DE"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: { kind: "fixedPerTransaction", amount: 1 },
        withdraw: { kind: "fixedPerTransaction", amount: 1 },
      },
    },
    {
      id: "mexem",
      name: "Mexem",
      website: "https://www.mexem.com",
      type: "broker",
      is_bank: false,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: {
          kind: "tieredTransaction",
          percentRatePct: 0.06,
          minimumAmount: 1,
        },
        withdraw: {
          kind: "tieredTransaction",
          percentRatePct: 0.06,
          minimumAmount: 1,
        },
      },
    },
    {
      id: "etoro",
      name: "eToro",
      website: "https://www.etoro.com",
      type: "broker",
      is_bank: false,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: { deposit: null, withdraw: null },
    },
    {
      id: "trading-212",
      name: "Trading 212",
      website: "https://www.trading212.com",
      type: "broker",
      is_bank: false,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: { deposit: null, withdraw: null },
    },
    {
      id: "interactive-brokers",
      name: "Interactive Brokers",
      website: "https://www.interactivebrokers.com",
      type: "broker",
      is_bank: false,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: {
          kind: "tieredTransaction",
          percentRatePct: 0.05,
          minimumAmount: 3,
        },
        withdraw: {
          kind: "tieredTransaction",
          percentRatePct: 0.05,
          minimumAmount: 3,
        },
      },
    },
    {
      id: "revolut-standard-plus",
      name: "Revolut (Standard / Plus)",
      website: "https://www.revolut.com",
      type: "broker",
      is_bank: true,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: {
          kind: "tieredTransaction",
          percentRatePct: 0.1,
        },
        withdraw: {
          kind: "tieredTransaction",
          percentRatePct: 0.1,
        },
      },
    },
    {
      id: "revolut-premium",
      name: "Revolut (Premium)",
      website: "https://www.revolut.com",
      type: "broker",
      is_bank: true,
      countries: ["BE", "NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [{ kind: "fixed", amount: 9.99 }] },
      transactions: {
        deposit: {
          kind: "freeFirstNThenPercent",
          freeTransactionCount: 5,
          percentRatePct: 0.1,
        },
        withdraw: {
          kind: "freeFirstNThenPercent",
          freeTransactionCount: 5,
          percentRatePct: 0.1,
        },
      },
    },
    {
      id: "saxo-autoinvest-nl",
      name: "Saxo (Autoinvest) NL",
      website: "https://www.home.saxo",
      type: "broker",
      is_bank: false,
      transactionsPerMonth: 1,
      countries: ["NL"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "",
      tax_BE_TOB_service: "Nee",
      tax_BE_roerende_voorheffing: "",
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            ratePct: 0.12,
            frequency: "monthly",
            basis: "avg",
            maximumMonthly: 40,
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: {
          kind: "tieredTransaction",
          percentTiers: {
            tiers: [
              { upTo: 200000, ratePct: 0.08 },
              { upTo: 1000000, ratePct: 0.05 },
              { upTo: null, ratePct: 0.03 },
            ],
            inclusiveUpper: false,
            apply: "highest",
          },
          minimumAmount: 2,
        },
      },
    },
    {
      id: "saxo-autoinvest-be",
      name: "Saxo (Autoinvest) BE",
      website: "https://www.home.saxo",
      type: "broker",
      is_bank: false,
      transactionsPerMonth: 1,
      countries: ["BE"],
      lastUpdated: "18-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: { underlyingAnnualPct: 0.21 },
      underlyingCostBasis: "avg",
      fees: { components: [{ kind: "fixed", amount: 2 }] },
      transactions: {
        deposit: null,
        withdraw: {
          kind: "tieredTransaction",
          percentTiers: {
            tiers: [
              { upTo: 200000, ratePct: 0.08 },
              { upTo: 1000000, ratePct: 0.05 },
              { upTo: null, ratePct: 0.03 },
            ],
            inclusiveUpper: false,
            apply: "highest",
          },
          minimumAmount: 2,
        },
      },
    },
    {
      id: "rebel",
      name: "Re=Bel",
      website: "https://www.belfius.be",
      type: "broker",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: null,
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      service_recurring_investing: false,
      service_fractional_investing: false,
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: { kind: "rebelTieredFixed" },
        withdraw: { kind: "rebelTieredFixed" },
      },
    },
    {
      id: "belfius-flex-invest-candriam",
      name: "Belfius (Flex Invest Plan - Candriam Equities L Global Demography)",
      website: "https://www.belfius.be",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "20-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: 99, // Candriam vermeldt een equity exposure van 98,98%.
      fbi: false,
      risk_indicator: 4,
      dividend: "accumulating",
      strategy: "active",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "Nee",
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      tax_BE_TOB_percentage: 1.32,
      tax_BE_TOB_max: 4000, // Voor het tarief van 1,32% (dat geldt voor de verkoop van kapitalisatiefondsen en de aan- en verkoop van bepaalde kapitalisatie-ETF's) ligt het maximum op €4.000 per transactie.
      tax_BE_TOB_deposit: false, // De wet ziet het instappen in een fonds als een "inschrijving op een nieuwe uitgifte", en dat is vrijgesteld van beurstaks. Pas als je ze weer verkoopt aan de bank, wordt de taks aangerekend.
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: false,
      overrides: { underlyingAnnualPct: 2.17 },
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: {
          kind: "tieredTransaction",
          percentTiers: {
            tiers: [
              { upTo: 50000, ratePct: 2.5 },
              { upTo: 125000, ratePct: 1.75 },
              { upTo: 250000, ratePct: 1 },
              { upTo: null, ratePct: 0.75 },
            ],
            inclusiveUpper: false,
            apply: "highest",
          },
        },
        withdraw: null,
      },
    },
    {
      id: "bnp-paribas-b-strategy-global-aggressive-flexinvest",
      name: "BNP Paribas (B Strategy Global Aggressive) Flexinvest",
      website: "https://www.bnpparibasfortis.be",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "24-02-2026",
      minimumInleg: null,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 95,
      fbi: false,
      risk_indicator: 4,
      dividend: "accumulating",
      strategy: "active",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "Nee", // geen bonds hierin toch?
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      service_recurring_investing: true,
      service_fractional_investing: true,
      tax_BE_TOB_percentage: 1.32,
      tax_BE_TOB_max: 4000, // 1,32% TOB voor kapitalisatiedeelbewijzen bij verkoop, max. €4.000 per verrichting
      tax_BE_TOB_deposit: false, // Geen TOB bij instap
      tax_BE_TOB_withdrawal: true,
      overrides: { underlyingAnnualPct: 2.02 }, // Checked at 26-03-2026
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: { kind: "tieredTransaction", percentRatePct: 2.5 }, // Checked at 26-03-2026
        withdraw: null,
      },
    },
    {
      id: "bolero-etf-playlist",
      name: "Bolero (ETF Playlist)",
      website: "https://www.bolero.be",
      type: "broker",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: null,
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      service_recurring_investing: false,
      service_fractional_investing: false,
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: { kind: "boleroEtfPlaylist" },
        withdraw: { kind: "boleroEtfPlaylist" },
      },
    },
    {
      // Keytrade onderliggende fondsen Beleggingsplan Gedurfd:
      // 10%	LU2809794220	1,8	0,25	2,05
      // 20%	LU0203975437	1,46	0,48	1,94
      // 20%	LU0248172537	1,84	0,81	2,65
      // 20%	LU0093502762	1,82	0,11	1,93 AC
      // 10%	LU0386882277	2	0,09	2,09
      // 20%	LU0172125329	1,81	0,1	1,91
      // 2,100
      // alle fondsen zitten bijna 100% in aandelen.
      id: "keytrade-bank-keyplan-gedurfd",
      name: "Keytrade Bank (Keyplan Beleggingsplan Gedurfd)",
      website: "https://www.keytradebank.be",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "19-02-2026",
      minimumInleg: 25,
      duurzaamheid: "Grijs",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null, // Niet bekend, want je kunt niet zomaar een gewogen gemiddelde nemen van de onderliggende fondsen.
      dividend: "accumulating",
      strategy: "active",
      underlyingDepositPercentage: 0, // Onduidelijk, afgaande op letter na namen fondsen waar het heen gaat lijkt het beide) https://www.keytradebank.be/nl/investeren/keyplan/creeer-uw-beleggingsplan?initialAmount=0&recurringAmount=25&periodicity=2&selectedProfile=A
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "Nee",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      service_recurring_investing: false,
      service_fractional_investing: false,
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: true,
      overrides: { underlyingAnnualPct: 2.1 },
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      // Met Flexinvest kun je automatisch hun fondsen kopen, dat kan niet met ETF's
      id: "kbc-beleggingsplan-equity-fund-world",
      name: "KBC (Beleggingsplan - Equity Fund World)",
      website: "https://www.kbc.be",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "24-02-2026",
      minimumInleg: null,
      duurzaamheid: "Grijs",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: "accumulating",
      strategy: "active",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "Nee",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      service_recurring_investing: true,
      service_fractional_investing: true,
      // Waarom: KBC Equity Fund World is een BEVEK die geen dividenden uitkeert (“dividenden: nee”), dus dit is een kapitalisatie-fonds.
      // KBC's eigen overzicht van beurstaksen zegt dat voor een kapitalisatiebevek (FSMA-erkend) de TOB 1,32% bedraagt bij aankoop én bij verkoop (max. €4.000 per verrichting).
      tax_BE_TOB_percentage: 1.32,
      tax_BE_TOB_max: 4000,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: { underlyingAnnualPct: 1.9 },
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: { kind: "tieredTransaction", percentRatePct: 3 },
        withdraw: null,
      },
    },
    {
      id: "crelan-beleggingsplan-amundi-global-equity-ae-c",
      name: "Crelan (Beleggingsplan - Amundi Funds Global Equity - AE (C))",
      website: "https://www.crelan.be",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "01-04-2026",
      minimumInleg: 25,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: 4,
      dividend: "accumulating",
      strategy: "active",
      underlyingDepositPercentage: 4.5,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja", // staat nergens, maar is in principe verplicht in België
      tax_BE_roerende_voorheffing: "Nee",
      tax_BE_roerende_voorheffing_service: null,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      tax_BE_TOB_percentage: 1.32, // AANNAME - Geen fiscale info op Crelan website te vinden.
      tax_BE_TOB_max: 4000, // AANNAME - Geen fiscale info op Crelan website te vinden.
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: null,
      overrides: { underlyingAnnualPct: 2.51 }, // 1,93% + 0,58%
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: { deposit: null, withdraw: null },
    },
    {
      id: "argenta-beleggingsplan-kernfonds-dynamic-growth-r-cap",
      name: "Argenta (Beleggingsplan - Kernfonds Dynamic Growth - R Cap)",
      website: "https://www.argenta.be",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "01-04-2026",
      minimumInleg: 25,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100, // Kan wisselen
      fbi: false,
      risk_indicator: 4,
      dividend: "accumulating",
      strategy: "active",
      underlyingDepositPercentage: 1,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja", // staat nergens, maar is in principe verplicht in België
      tax_BE_roerende_voorheffing: "Nee",
      tax_BE_roerende_voorheffing_service: null,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee",
      tax_BE_TOB_percentage: 1.32,
      tax_BE_TOB_max: 4000,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: true,
      service_recurring_investing: true,
      service_fractional_investing: null,
      overrides: { underlyingAnnualPct: 1.97 }, // 1,82% + 0,15%
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: { deposit: null, withdraw: null },
    },
    {
      // https://bux.com/nl/tarieven/
      id: "bux-basic",
      name: "Bux Basic",
      website: "https://www.getbux.com",
      type: "broker",
      is_bank: false,
      countries: ["NL", "BE", "DE", "FR", "IT", "ES", "AT", "IE"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: null,
      service_recurring_investing: true,
      service_fractional_investing: true,
      // Belgische TOB op inleg (storting/initiële inleg)
      tax_BE_TOB_percentage: 0.12, // 0,12% per inleg
      tax_BE_TOB_max: 1300, // maximaal 1.300 EUR per maand (cap)
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "fixed", amount: 0 },
          { kind: "percentOfBase", ratePct: 0.2, frequency: "monthly", basis: "avg" },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: { kind: "fixedPerTransaction", amount: 0.99},
      },
    },
    {
      // https://bux.com/nl/tarieven/
      id: "bux-plus",
      name: "Bux Plus",
      website: "https://www.bux.com",
      type: "broker",
      is_bank: false,
      countries: ["NL", "BE", "DE", "FR", "IT", "ES", "AT", "IE"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: null,
      service_recurring_investing: true,
      service_fractional_investing: true,
      // Belgische TOB op inleg (storting/initiële inleg)
      tax_BE_TOB_percentage: 0.12, // 0,12% per inleg
      tax_BE_TOB_max: 1300, // maximaal 1.300 EUR per maand (cap)
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "fixed", amount: 2.99 },
          {
            kind: "percentOfBase",
            frequency: "monthly",
            basis: "avg",
            tiers: {
              inclusiveUpper: true,
              apply: "marginal",
              tiers: [
                { upTo: 250000, ratePct: 0 },
                { upTo: null, ratePct: 0.1 },
              ],
            },
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: { kind: "fixedPerTransaction", amount: 0.99},
      },
    },
    {
      // https://bux.com/nl/tarieven/
      id: "bux-prime",
      name: "Bux Prime",
      website: "https://www.bux.com",
      type: "broker",
      is_bank: false,
      countries: ["NL", "BE", "DE", "FR", "IT", "ES", "AT", "IE"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: null,
      service_recurring_investing: true,
      service_fractional_investing: true,
      // Belgische TOB op inleg (storting/initiële inleg)
      tax_BE_TOB_percentage: 0.12, // 0,12% per inleg
      tax_BE_TOB_max: 1300, // maximaal 1.300 EUR per maand (cap)
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "fixed", amount: 7.99 },
          {
            kind: "percentOfBase",
            frequency: "monthly",
            basis: "avg",
            tiers: {
              inclusiveUpper: true,
              apply: "marginal",
              tiers: [
                { upTo: 500000, ratePct: 0 },
                { upTo: null, ratePct: 0.1 },
              ],
            },
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: { kind: "fixedPerTransaction", amount: 0.99},
      },
    },
    {
      // https://direct.vaneck.com/kosten
      // https://direct.vaneck.com/help/verschil-vaneck-vaneckdirect --> €100 minimale inleg
      // Ze vragen ook een symblogische €1 als je geld opneemt vóór je doel bereikt. Die is niet meegenomen.
      id: "vaneck-direct-zeer-offensief",
      name: "VanEck Direct (Zeer offensief)",
      website: "https://www.vaneck.com",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: 100,
      // 95% SFDR Artikel 8, 5% Artikel 6
      duurzaamheid: "Grijs",
      aandelenpercentage: 90,
      fbi: true,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.22,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "percentOfBase", ratePct: 0.5, frequency: "monthly", basis: "avg" },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      // https://www.semmiewealth.nl/kosten/
      // De fondskosten vermelden ze niet langer, daarom vasthouden aan de eerder gemelde kosten.
      id: "semmiewealth-growth-solid-zeer-offensief",
      name: "SemmieWealth (Growth Solid Zeer offensief)",
      website: "https://www.semmiewealth.nl",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: 1000,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.21,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "fixed", amount: 3.99 },
          { kind: "percentOfBase", ratePct: 0.85, frequency: "monthly", basis: "avg", minimumAnnual: 85 },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      // https://www.semmiewealth.nl/kosten/
      // De fondskosten vermelden ze niet langer, daarom vasthouden aan de eerder gemelde kosten.
      id: "semmiewealth-premium-solid-zeer-offensief",
      name: "SemmieWealth (Premium Solid Zeer offensief)",
      website: "https://www.semmiewealth.nl",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: null,
      minimumInleg: 500000,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.21,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "fixed", amount: 3.99 },
          { kind: "percentOfBase", ratePct: 0.75, frequency: "monthly", basis: "avg" },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "raisin-zeer-offensief",
      name: "Raisin (Zeer offensief)",
      website: "https://www.raisin.com",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: 25,
      duurzaamheid: "Grijs",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.13,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            frequency: "monthly",
            basis: "avg",
            tiers: {
              inclusiveUpper: false,
              apply: "highest",
              tiers: [
                { upTo: 100000, ratePct: 0.46 },
                { upTo: 250000, ratePct: 0.32 },
                { upTo: 500000, ratePct: 0.29 },
                { upTo: null, ratePct: 0.25 },
              ],
            },
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "peaks-avontuurlijk-2-jaar",
      name: "Peaks (Avontuurlijk, 2 jaar)",
      website: "https://peaks.nl",
      type: "fondsaanbieder",
      countries: ["NL", "DE"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Grijs", // 93% SFDR Article 8, 7% SFDR Article 6
      aandelenpercentage: 90,
      fbi: false,
      risk_indicator: null,
      dividend: "accumulating",
      strategy: "passive",
      underlyingDepositPercentage: 0, // de 90% aandelen ETF's volgen een index, obligaties = ?
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.2,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "fixed", amount: 1.59 },
          { kind: "percentOfBase", ratePct: (0.04 * 12), frequency: "monthly", basis: "avg" }, // Dit was 0,5% per jaar. Nu staat er 0,04% per maand, dat is 0,48% per jaar. Mogelijk ronden ze dit dus gunstig af
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "peaks-avontuurlijk-maandelijks",
      name: "Peaks (Avontuurlijk, maandelijks)",
      website: "https://peaks.nl",
      type: "fondsaanbieder",
      countries: ["NL", "DE"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Grijs", // 93% SFDR Article 8, 7% SFDR Article 6
      aandelenpercentage: 90,
      fbi: false,
      risk_indicator: null,
      dividend: "accumulating",
      strategy: "passive",
      underlyingDepositPercentage: 0, // de 90% aandelen ETF's volgen een index, obligaties = ?
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.2,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          { kind: "fixed", amount: 1.99 },
          { kind: "percentOfBase", ratePct: (0.04 * 12), frequency: "monthly", basis: "avg" }, // Dit was 0,5% per jaar. Nu staat er 0,04% per maand, dat is 0,48% per jaar. Mogelijk ronden ze dit dus gunstig af
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      // https://new.brandnewday.nl/beleggingsrekening/kosten-beleggingsrekening/
      id: "brand-new-day-modelportefeuille-zeer-offensief",
      name: "Brand New Day (Modelportefeuille Zeer offensief)",
      website: "https://www.brandnewday.nl",
      type: "fondsaanbieder",
      is_bank: false,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: true,
      risk_indicator: null,
      dividend: null,
      strategy: "passive",
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.17,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            ratePct: 0.34,
            frequency: "quarterly", // jaarbasis, per kwartaal in rekening over gemiddelde saldo
            basis: "avg",
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      // https://www.knab.nl/particulier/beleggen/beheerd-beleggen/kosten
      id: "knab-beheerd-zeer-offensief",
      name: "Knab (Beheerd Beleggen Zeer Offensief)",
      website: "https://www.knab.nl",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.18,
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "avg",
            frequency: "quarterly",
            tiers: {
              inclusiveUpper: false,
              apply: "highest",
              tiers: [
                { upTo: 25000, ratePct: 0.6 },
                { upTo: null, ratePct: 0.5 },
              ],
            },
          },
        ],
      },
      transactions: {
        deposit: { kind: "tieredTransaction", percentRatePct: 0.1 },
        withdraw: { kind: "tieredTransaction", percentRatePct: 0.1 },
      },
    },
    {
      // KID: BE6282434354 -> distributing, dus 0,12 TOB?. 
      id: "ing-belgium-easy-invest",
      name: "ING België (Easy Invest - Personal Portfolio Dynamic)",
      website: "https://www.ing.be",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "24-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: 95, // 5,2% uit obligaties per 28 februari 2026.
      fbi: false,
      risk_indicator: 3,
      dividend: "distributing",
      strategy: "active",
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: "Ja",
      tax_BE_roerende_voorheffing_service: "Ja",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "Nee", // Ze geven het automatisch aan wanneer nodig (dus <90% aandelen)
      service_recurring_investing: true,
      service_fractional_investing: true,
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: { underlyingAnnualPct: 2.2 },
      underlyingCostBasis: "avg",
      fees: { components: [] },
      transactions: {
        deposit: { kind: "tieredTransaction", percentRatePct: 2.5 },
        withdraw: null,
      },
    },
    {
      id: "ing-self-invest",
      name: "ING België (Self Invest)",
      website: "https://www.ing.be",
      type: "broker",
      is_bank: true,
      countries: ["BE"],
      lastUpdated: "24-02-2026",
      minimumInleg: null,
      duurzaamheid: null,
      aandelenpercentage: null,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_TOB: "Ja",
      tax_BE_TOB_service: "Ja",
      tax_BE_roerende_voorheffing: null,
      tax_BE_roerende_voorheffing_service: "Nee",
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_effectentaks: "Ja",
      tax_BE_reynderstaks: "",
      service_recurring_investing: false, // kan alleen in hun eigen fondsen, die staan al bij easy invest
      service_fractional_investing: false,
      tax_BE_TOB_percentage: 0.12,
      tax_BE_TOB_max: 1300,
      tax_BE_TOB_deposit: true,
      tax_BE_TOB_withdrawal: true,
      overrides: {},
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            ratePct: 0.24,
            frequency: "monthly",
            basis: "avg",
            minimumMonthly: 0.25,
          },
        ],
      },
      transactions: {
        deposit: { kind: "tieredTransaction", percentRatePct: 0.35, minimumAmount: 1 },
        withdraw: { kind: "tieredTransaction", percentRatePct: 0.35, minimumAmount: 1 },
      },
    },
    {
      id: "ing-eenvoudig-beleggen-dynamic",
      name: "ING (Eenvoudig Beleggen Dynamic)",
      website: "https://www.ing.nl",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: 10,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 90,
      fbi: false,
      risk_indicator: 3,
      dividend: "accumulating",
      strategy: "active", // ze passen weging ETF's naar eigen inzicht aan.
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.7, // 0,66% beheerskosten + 0,04% transactiekosten
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            ratePct: 0.4, // servicekosten
            frequency: "quarterly",
            basis: "avg",
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "abn-amro-begeleid-esg-zeer-offensief",
      name: "ABN AMRO (Begeleid Beleggen ESG Profielfonds Zeer offensief)",
      website: "https://www.abnamro.nl",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: 20,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 90,
      fbi: false,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 1.12, // ABN AMRO Profielfondsen lopende kosten en transactiekosten per 1-4-2025: 0,84% + 0,28%
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            ratePct: 0.25, // per jaar
            frequency: "quarterly",
            basis: "end",
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "rabobank-rabo-simpelbeleggen",
      name: "Rabobank (Rabo SimpelBeleggen)",
      website: "https://www.rabobank.nl",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: 10,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 100, //  Je kunt zelf je verdeling kiezen van 0 tot 100%
      fbi: true,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.1, /* 0,1% bij aandelenfonds, 0,15% bij obligatiefonds */
      },
      underlyingCostBasis: "avg",
      minimumQuarterlyFee: 5, // 0,5% per kwartaal, min. €5 per kwartaal
      fees: {
        components: [
          {
            kind: "percentOfBase",
            ratePct: 0.5, /* Dus 0,125% per kwartaal */
            frequency: "quarterly",
            basis: "avg",
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
    {
      id: "rabobank-rabo-beheerd-basis-zeer-offensief",
      name: "Rabobank (Rabo Beheerd beleggen Basis zeer offensief)",
      website: "https://www.rabobank.nl",
      type: "fondsaanbieder",
      is_bank: true,
      countries: ["NL"],
      lastUpdated: "10-03-2026",
      minimumInleg: null,
      duurzaamheid: "Lichtgroen",
      aandelenpercentage: 90,
      fbi: true,
      risk_indicator: null,
      dividend: null,
      strategy: null,
      underlyingDepositPercentage: 0,
      underlyingWithdrawalPercentage: 0,
      tax_BE_roerende_voorheffing_percentage: null,
      tax_BE_TOB_deposit: false,
      tax_BE_TOB_withdrawal: false,
      service_recurring_investing: true,
      service_fractional_investing: true,
      overrides: {
        underlyingAnnualPct: 0.15, /* van Rabobank helpdesk chat ontvangen, screenshot opgeslagen */
      },
      underlyingCostBasis: "avg",
      fees: {
        components: [
          {
            kind: "percentOfBase",
            basis: "avg",
            frequency: "quarterly",
            minimumQuarterly: 5,
            tiers: {
              inclusiveUpper: false,
              apply: "marginal",
              tiers: [
                { upTo: 100000, ratePct: 0.06*4 },
                { upTo: 500000, ratePct: 0.3*4 },
                { upTo: 2000000, ratePct: 0.01*4 },
                { upTo: null, ratePct: 0.0025*4 },
              ],
            },
          },
          {
            kind: "percentOfBase",
            basis: "avg",
            frequency: "quarterly",
            tiers: {
              inclusiveUpper: false,
              apply: "marginal",
              tiers: [
                { upTo: 100000, ratePct: 0.10*4 },
                { upTo: null, ratePct: 0.09*4 },
              ],
            },
          },
        ],
      },
      transactions: {
        deposit: null,
        withdraw: null,
      },
    },
  ];

  /** Landen die in het filter dropdown verschijnen. null = alle landen uit providers. */
  const FILTER_COUNTRIES = ["NL", "BE"];

  const DEFAULTS = {
    type: "broker", // informational only; provider determines type
    transactionsPerMonth: 3,
    underlyingDepositPercentage: 0,
    underlyingWithdrawalPercentage: 0,
    beginbedrag: 0,
    maandbedrag: 150,
    underlyingAnnualPct: 0.2084,
    years: 10,
    zonderOnderliggendeKosten: false,
    zonderKosten: false,
    zonderTOB: false,
  };

  window.BerekenProviders = {
    PROVIDERS,
    DEFAULTS,
    FILTER_COUNTRIES,
  };
})();

